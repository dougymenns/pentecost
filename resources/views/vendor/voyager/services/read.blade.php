...
@section('javascript')
    ...
    <script>
        ...
        var session_input = $('div.panel-body:last p').html();
        var session_rows = session_input.split(',');
        var session_details = [];

        $('div.panel-body:last').html('<table class="table"><tr><th>Name</th><th>Start Time</th><th>End Time</th></tr></table>');

        for(var a=0; a < (session_rows.length - 1); a++)
        {
            var session_fields = session_rows[a].split(';');

            $('div.panel-body:last table').append(
                '<tr><td>' + session_fields[1] + '</td><td>' + session_fields[2] + '</td><td>' + session_fields[3] + '</td></tr>'
            );
        }
